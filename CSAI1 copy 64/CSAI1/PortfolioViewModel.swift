...
class PortfolioViewModel: ObservableObject {
    ...
    func fetchHoldingsFrom3Commas() {
        // Call the ThreeCommasService to list connected accounts.
        ThreeCommasService.shared.listAccounts { [weak self] result in
            DispatchQueue.main.async {
                switch result {
                case .success(let json):
                    // Assuming the API returns an array of account dictionaries
                    if let accounts = json as? [[String: Any]] {
                        var newHoldings: [Holding] = []
                        // Iterate over each account and create a Holding (adjust keys as per actual API response)
                        for account in accounts {
                            // Example: Assuming account dictionary has keys: "coinName", "coinSymbol", "balance", "price"
                            if let coinSymbol = account["coinSymbol"] as? String,
                               let coinName = account["coinName"] as? String,
                               let balance = account["balance"] as? Double,
                               let price = account["price"] as? Double {
                                let holding = Holding(
                                    coinName: coinName,
                                    coinSymbol: coinSymbol,
                                    quantity: balance,
                                    currentPrice: price,
                                    costBasis: price, // Default cost basis to current price; adjust as needed
                                    imageUrl: nil,
                                    isFavorite: false,
                                    dailyChange: 0.0,
                                    purchaseDate: Date()
                        ...
                        print("Unexpected data format for accounts.")
                    }
